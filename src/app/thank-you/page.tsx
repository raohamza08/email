  ...
  return (
    <main className="container">
      ...
      <section className="card">
        ...
        {/* Shimmer button (optional link back to site) */}
        ...
        * { box-sizing: border-box; }
        ...
        .bg-grid{
          ...
          background:
            ...
            repeating-linear-gradient(0deg, rgba(255,255,255,0.06), rgba(255,255,255,0.06) 1px, transparent 1px, transparent 40px),
            repeating-linear-gradient(90deg, rgba(255,255,255,0.06), rgba(255,255,255,0.06) 1px, transparent 1px, transparent 40px);
          opacity: .25;
        ...
        .glow-orb{
          ...
          filter: blur(30px); opacity: .35; mix-blend-mode: screen; z-index: -1;
        }
        .orb-1{ background: radial-gradient(circle at 30% 30%, #22d3ee, transparent 60%);
                top: -8vmin; left: -6vmin; animation: float1 14s ease-in-out infinite; }
        .orb-2{ background: radial-gradient(circle at 70% 70%, #60a5fa, transparent 60%);
                bottom: -10vmin; right: -8vmin; animation: float2 16s ease-in-out infinite; }
        ...
        /* Animated border ring using mask */
        .border-anim{
        ...
        .check{
          ...
          width: 94px; height: 94px; margin: 0 auto 14px;
          border-radius: 50%;
          background: radial-gradient(circle at 35% 30%, rgba(34,211,238,.35), transparent 60%);
          position: relative;
        }
        ...
        .tick{
          fill: none; stroke: #22d3ee; stroke-width: 5; stroke-linecap: round; stroke-linejoin: round;
          filter: drop-shadow(0 0 8px rgba(34,211,238,.65));
          stroke-dasharray: 48; stroke-dashoffset: 48;
          animation: draw 700ms ease-out forwards .5s;
        }
        ...
        .btn{
          ...
          background:
            linear-gradient(180deg, #8be8ff, #5ed6ef 45%, #44cde8 60%, #18b8d6 100%);
          box-shadow: 0 10px 28px rgba(6,182,212,0.35), inset 0 1px 0 rgba(255,255,255,.6);
          outline: none;
          border: 0;
        ...
        .confetti span{
          --size: calc(6px + (var(--i) % 3) * 2px);
          position: absolute; left: 50%; top: 42%;
          width: var(--size); height: var(--size);
          background: hsl(calc((var(--i) * 37) % 360), 90%, 60%);
          transform: translate(-50%, -50%) rotate(0deg);
          border-radius: 2px;
          animation: pop 900ms ease-out forwards, fall 1400ms cubic-bezier(.2,.7,.2,1) forwards;
          animation-delay: calc(20ms * var(--i));
          opacity: 0.95;
        }
        @keyframes pop{
          to { transform: translate(-50%, -50%) rotate(180deg) scale(1.3); }
        }
        @keyframes fall{
          to {
            transform:
              translate(calc(-50% + (var(--i) - 14) * 12px), calc(-50% + 160px + (var(--i) % 5) * 8px))
              rotate(calc((var(--i) * 23) * 1deg));
            opacity: 0;
          }
        }
